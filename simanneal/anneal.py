...
class Annealer(object):
    ...
    def __init__(self, initial_state=None, E = 0, step = 0, print_info = True):
        if initial_state is not None:
            ...
            assert not E == 0
            self.E = E
            self.obtain_optimal_steps = -1 # obtaining the optimal solution with steps
    ...
    def default_update(self, epoch, step_in_epoch, step, T, E, acceptance, improvement):
        ...
        if self.print_info:
            if step == 0:
                print('\n NSA Epoch Step Temperature        Energy    Accept   Improve     Elapsed   Remaining',
                    file=sys.stderr)
                print('\r{NSA: 4.0f} {Epoch: 5.0f} {Step: 4.0f} {Temp:12.5f}  {Energy:12.2f}                      {Elapsed:s}            '
                    .format(NSA = 1 if config.SA_with_RL else 0,
                            Epoch = epoch,
                            Step = step_in_epoch,  
                            Temp=T,
                            Energy=E,
                ...
                print('\r{NSA: 4.0f} {Epoch: 5.0f} {Step: 4.0f} {Temp:12.5f}  {Energy:12.2f}   {Accept:7.2%}   {Improve:7.2%}  {Elapsed:s}  {Remaining:s}'
                    .format(NSA = 1 if config.SA_with_RL else 0,
                            Epoch = epoch,
                            Step = step_in_epoch, 
                            Temp=T,
    ...
    def anneal(self, epoch, step_in_epoch, f_write):
        ...
        if self.updates > 0:
            ...
            self.update(epoch, step_in_epoch, step, T, self.E, None, None)
        if f_write:
            if config.SA_with_RL:
                f_write.write('NSA:\n')
            else:
                f_write.write('SA:\n')
            f_write.write('{}, {}\n'.format(0, self.best_energy))
        ...
        while step < self.steps and not self.user_exit:
            ...
            if dE > 0.0 and math.exp(-dE / T) < random.random():
                ...
                if self.E < self.best_energy:
                    self.best_state = self.copy_state(self.state)
                    self.best_energy = self.E
                    self.obtain_optimal_steps = step
                    duration = time.time() - self.start
                    if duration >= config.timelimitvalue:
                        break
                    if f_write:
                        f_write.write('{}, {}\n'.format(round(time.time() - self.start, 2), self.best_energy))

            if self.updates > 1:
                if (step // updateWavelength) > ((step - 1) // updateWavelength):
                    self.update(epoch, step_in_epoch,
                        step, T, self.E, accepts / trials, improves / trials)
                    trials = accepts = improves = 0
        ...
        return self.best_state, self.best_energy, self.obtain_optimal_steps
